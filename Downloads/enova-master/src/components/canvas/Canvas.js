...
const Canvas = () => {
  ...
  const pan = (direction) => {
    const { scale, x, y } = getTransformParameters(canvas);
    let dx = 0,
      dy = 0;
    switch (direction) {
      case "left":
    ...
    canvas.style.transform = getTransformString(scale, x + dx, y + dy);
  };
  ...
  const rotate = (ii) => {
    aux = aux + ii;
    console.log(ii);
    canvas.style.transform = "rotate(" + aux + "deg) ";
  };
  ...
  return (
    <div className="container">
      <div className="container row">
        ...
        <button id="rotate" onClick={()=>{rotate(5)}}>rotate</button>
